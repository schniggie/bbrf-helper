...
while IFS="," read -r url postbody; do echo "Testing POST URL: $url"; httpx -include-response -json -silent -u '$url' -body '$postbody' | httpx2bbrf -s rad -t postbody:'$postbody' --show-new; done < <(cat rad/post.csv)
